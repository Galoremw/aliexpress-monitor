...
商品采集请求可以附带 `historical_sales`。扩展优先读取页面上可见且明确呈现为
销量/订单的日期表格；如果 IXSPY 只提供图表，则有限次悬停可见图表并读取
tooltip 中的完整日期和值。每个点必须声明 `value_type`：

- `daily_increment`：该日期的公开数据推算销量增量。
- `cumulative_total`：该日期的公开累计销量，仅在相邻日期连续且数值不下降时计算差值。

后端会把原始点保存在 ProductSnapshot.raw_data，并把可验证的点写入日指标。
没有可见表格或可见 tooltip 时不会猜测画布像素、隐藏状态或私有请求，仍使用普通快照差值逻辑。
