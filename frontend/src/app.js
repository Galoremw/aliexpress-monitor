...
function isDedicatedWorkerPage() { return new URLSearchParams(location.search).get("dianxiaomi_worker") === "1"; }
function shell(title, content) {
  const workerMode = isDedicatedWorkerPage();
  document.title = workerMode ? `${title} · 隔离 Chrome 工作模式` : `${title} · AliExpress 竞品监控`;
  return `<header class="topbar"><a class="brand" href="#/dashboard">AliExpress 竞品监控</a>${workerMode ? '<span class="worker-mode-badge">隔离 Chrome 工作模式</span>' : ''}<nav><a href="#/dashboard">监控台</a><a href="#/stores">店铺</a><a href="#/products">商品</a><a href="#/manual">人工处理</a><button class="nav-logout" data-action="logout">退出登录</button></nav></header><main><div class="page-title">${title}</div>${content}</main>`;
}
...
const dianxiaomiStatusLabels = { QUEUED: "待发送", CLAIMED: "扩展已领取", OPENED: "后台页面已打开", FILLED: "链接已填入", COLLECTING: "店小秘采集中", SUCCEEDED: "店小秘已完成", FAILED: "失败", NEEDS_CONFIRMATION: "需人工确认", CANCELED: "已取消" };
function escapeHtml(value) { return String(value ?? "").replace(/[&<>"']/g, (character) => ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" }[character])); }
function dianxiaomiNextAction(item) { if (item.status === "NEEDS_CONFIRMATION") { const message = String(item.error_message || ""); if (message.includes("协议")) return "请在店小秘页面勾选采集协议后点击“重新排队”"; if (message.includes("登录")) return "请在店小秘页面登录后点击“重新排队”"; return "请处理店小秘页面提示后点击“重新排队”"; } if (item.status === "FAILED") return "请检查店小秘页面后点击“重新排队”"; return ""; }
function dianxiaomiItem(item) { const status = dianxiaomiStatusLabels[item.status] || item.status; const statusClass = item.status === "SUCCEEDED" ? "success" : ["FAILED", "NEEDS_CONFIRMATION"].includes(item.status) ? "error" : ""; const action = ["NEEDS_CONFIRMATION", "FAILED"].includes(item.status) ? `<button class="button" data-action="dianxiaomi-resume" data-id="${item.id}">重新排队</button>` : ["QUEUED", "CLAIMED", "OPENED", "FILLED", "COLLECTING"].includes(item.status) ? `<button class="button" data-action="dianxiaomi-cancel" data-id="${item.id}">取消</button>` : ""; const title = escapeHtml(item.product_title || item.platform_product_id || `商品 #${item.product_id}`); const store = escapeHtml(item.store_name || `店铺 #${item.store_id}`); const url = escapeHtml(item.target_url); const error = item.error_message ? ` · ${escapeHtml(item.error_message)}` : ""; const nextAction = dianxiaomiNextAction(item); return `<article class="dianxiaomi-item"><div class="dianxiaomi-item-main"><strong class="dianxiaomi-item-title">${title}</strong><span class="dianxiaomi-item-meta">${store} · <a href="${url}" target="_blank" rel="noreferrer">${url}</a></span><span class="dianxiaomi-item-meta">${formatDate(item.requested_at)}${error}</span>${nextAction ? `<span class="dianxiaomi-item-hint">${escapeHtml(nextAction)}</span>` : ""}</div><div class="dianxiaomi-item-side"><span class="dianxiaomi-status ${statusClass}">${status}</span><span class="dianxiaomi-item-actions">${action}</span></div></article>`; }
function dianxiaomiPanel(status) { const values = status || {}; const items = values.items || values.latest || []; return `<section class="section dianxiaomi-band"><div class="section-heading"><h2>店小秘采集队列</h2><span>通过已登录 Chrome 的正常页面提交</span></div><div class="browser-summary"><span>待发送：${values.queued || 0}</span><span>处理中：${values.processing || 0}</span><span>已提交：${values.submitted || 0}</span><span>失败：${values.failed || 0}</span><span>需人工确认：${values.needs_confirmation || 0}</span></div><p class="muted-value">${items[0] ? `${escapeHtml(items[0].product_title || items[0].platform_product_id || "商品")} · ${dianxiaomiStatusLabels[items[0].status] || items[0].status}` : "尚未提交店小秘采集任务。"}</p><div class="dianxiaomi-items">${items.map(dianxiaomiItem).join("") || empty("当前没有店小秘任务")}</div></section>`; }
