...
function dianxiaomiPanel(status) { const values = status || {}; return `<section class="section dianxiaomi-band"><div class="section-heading"><h2>店小秘采集队列</h2><span>通过已登录 Chrome 的正常页面提交</span></div><div class="browser-summary"><span>待发送：${values.queued || 0}</span><span>处理中：${values.processing || 0}</span><span>已提交：${values.submitted || 0}</span><span>失败：${values.failed || 0}</span><span>待人工确认：${values.needs_confirmation || 0}</span></div><p class="muted-value">${values.latest?.[0] ? `${values.latest[0].product_title || values.latest[0].platform_product_id} · ${values.latest[0].status}` : "尚未提交店小秘采集任务。"}</p></section>`; }
